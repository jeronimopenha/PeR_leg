...
def generate_connected_unbalanced_trees_by_num_vertexes(num_vertexes: list):
    ...
    for num_vertex in num_vertexes:
        ...
        connected = False
        while not connected:
            vertexes, edges_dfg, placement_c2n = graph_generator.generate_random_tree_graph(num_vertex,
                                                                                            [(0, 1), (0, -1), (1, 0),
                                                                                             (-1, 0)])
        ...
        path = Util.get_project_root() + f'/dot_db/graphs0_tree/'
<<<<<<<< HEAD:src/python/stat_scripts/generate_tree_graphs.py
...
generate_connected_unbalanced_trees_by_num_vertexes([15,16,18,20,23,25,27,30,33,35,40,42,44,47,50])
generate_connected_balanced_trees_by_heights([i for i in range(2,8)])
>>>>>>>> monte_carlo:generate_tree_graphs.py

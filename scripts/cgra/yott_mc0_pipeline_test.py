...
def run_connected_graphs():
    ...
    for arch_type in archs_types:
        ...
        for dot_path, dot_name in dots_list:
            ...
            pipeline_method = YOTTMC0Pipeline(per_graph, arch_type, distance_table_bits, make_shuffle, [0.5,0.8,0.3],3,10)
            if run_parallel:
            ...
            raw_report['method'] = 'MC'
            limiars_str ='0.5,0.8,0.3'
            raw_report['limiar'] = limiars_str
            formatted_report = Util.get_formatted_report(raw_report)
            Util.write_json(output_path, dot_name.replace('.dot', '') + f'-NA<{3}>-M<MC={limiars_str}>.dot', formatted_report)
            reports.append(formatted_report)
if __name__ == '__main__':
    per = PeRGraph(Util.get_project_root() + "/dot_db/connected/t.dot", "t.dot")
    # yott = YOTTMC0Pipeline(per, ArchType.MESH, 4, True,  [0.5,0.8,0.3], 3, 10).run_single(1000)

...
import src.old.python.hw.sa_fsm.sa_components as _sa
import old.sa_pipeline.util as _u
import src.old.python.hw.sa_aws as _aws
import src.old.python.hw.sa_fsm.sa_components as _comp
...
def create_sa_aws_test_bench(comp: _comp.SAComponents) -> str:
    ...
    aws = _aws.SaAws()
    aux = aws.get(comp.proj_graph, bus_width)
    m.Instance(aux, aux.name, par, con)

            ...
            if st4_cva[i] != -1:
                if self.arch_type == ArchType.ONE_HOP:
                    if st4_cas == st4_cva[i]:
                        dvas[i] = dist_one_hop(
                            get_line_column_from_cell(st4_cas, self.n_lines, self.n_columns),
                            get_line_column_from_cell(st4_cbs, self.n_lines, self.n_columns)
                        )
                    else:
                        dvas[i] = dist_one_hop(
                            get_line_column_from_cell(st4_cas, self.n_lines, self.n_columns),
                            get_line_column_from_cell(st4_cva[i], self.n_lines, self.n_columns)
                        )
                if self.arch_type == ArchType.MESH:
                    if st4_cas == st4_cva[i]:
                        dvas[i] = dist_manhattan(
                            get_line_column_from_cell(st4_cas, self.n_lines, self.n_columns),
                            get_line_column_from_cell(st4_cbs, self.n_lines, self.n_columns)
                        )
                    else:
            ...
            if st4_cvb[i] != -1:
                if self.arch_type == ArchType.ONE_HOP:
                    if st4_cbs == st4_cvb[i]:
                        ...
                        dvbs[i] = dist_one_hop(
                            get_line_column_from_cell(st4_cbs, self.n_lines, self.n_columns),
                            get_line_column_from_cell(st4_cvb[i], self.n_lines, self.n_columns)
                        )
                elif self.arch_type == ArchType.MESH:
                    if st4_cbs == st4_cvb[i]:
                        dvbs[i] = dist_manhattan(
                            get_line_column_from_cell(st4_cas, self.n_lines, self.n_columns),
                            get_line_column_from_cell(st4_cbs, self.n_lines, self.n_columns)
                        )
                    else:

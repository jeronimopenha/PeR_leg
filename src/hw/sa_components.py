...
class SAComponents:
    ...
    def create_sa_fsm_single_thread(self):
        ...
        DISTANCE_CALCULATOR = m.Localparam('DISTANCE_CALCULATOR', Int(4, fsm_sa.width, 10))
        SUM_REDUCTION = m.Localparam('SUM_REDUCTION', Int(5, fsm_sa.width, 10))
        DECISION = m.Localparam('DECISION', Int(6, fsm_sa.width, 10))
        CHANGES = m.Localparam('CHANGES', Int(7, fsm_sa.width, 10))
        END = m.Localparam('END', Int(8, fsm_sa.width, 10))
        m.EmbeddedCode('// #####')
        ...
        m.Always(Posedge(clk))(
            If(rst)(
                ...
                Case(fsm_sa)(
                    When(SELECT_CELLS)(
                        If(ca == Int(n_cells_sqrt))(
                            ca(Int(0, ca.width, 10)),
                            If(cb == Int(n_cells_sqrt))(
                                cb(Int(0, cb.width, 10)),
                                fsm_sa(END)
                            ).Else(

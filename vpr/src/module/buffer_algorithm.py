def create_buffer(g, dict_tree, dict_breadth, vector_dic_cost, strategy):
    OPEN, CLOSED, OUTS = [[],[]], [[],[]], []
        ...
        while len(OPEN[k]) > 0 :
            ...
            for son in list(g.successors(dad)):
                ...
                if son not in OPEN[k] and son not in CLOSED[k]:
                    OPEN[k].append(son)
                for i in range(len(vector_dic_cost)):
                    cost_dad = vector_dict_tree[i][dad] + vector_dic_cost[i][key][strategy] + 1
    ...
    while len(OUTS) > 0:
        ...
        for son in list(g.predecessors(dad)):
            ...
            for i in range(len(vector_dic_cost)):
                ...
                buffer[i][key] = (vector_dict_tree[i][dad] - dict_tree[dad]) - (vector_dict_tree[i][son] - dict_tree[son]) - vector_dic_cost[i][key][strategy]
                
    return buffer, vector_dict_tree 

def create_buffer_inverse(g, dict_tree, dict_breadth, vector_dic_cost, strategy):
    OPEN, CLOSED, OUTS = [[],[]], [[],[]], []
        ...
        while len(OPEN[k]) > 0 :
            ...
            for dad in list(g.predecessors(son)):
                ...
                if dad not in OPEN[k] and dad not in CLOSED[k]:
                    OPEN[k].append(dad)
                for i in range(len(vector_dic_cost)):
                    cost_son = vector_dict_tree[i][son] + vector_dic_cost[i][key][strategy] + 1
